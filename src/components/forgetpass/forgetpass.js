...
class Forget extends Component {
  ...
  valid = () => {
    const error = {};
    if (this.state.mobile.trim() === '') error.username = 'mobile is require';
    if (this.state.pass.trim() === '') error.pass = 'password is require';
    if (this.state.confirm !== this.state.pass)
      error.confirm = 'must enter the same pass';
    if (this.state.code.trim() === '') error.pass = 'coder is require';
    this.setState({ error });
